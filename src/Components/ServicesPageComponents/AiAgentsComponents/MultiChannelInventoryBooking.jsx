...
import { FaCheckCircle, FaBox, FaSyncAlt, FaBell, FaChartBar, FaCalendarAlt, FaChartLine, FaClock } from 'react-icons/fa'

const MultiChannelInventoryBooking = () => {
    ...
    <section className="relative py-16 md:py-24 bg-[var(--bg-color)] overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          >
            Multi-Channel Inventory & Booking Automation
          </motion.h1>
          <motion.p 
            className="text-sm md:text-base max-w-2xl mx-auto text-[var(--text-color)]/90 leading-relaxed"
            variants={sectionVariants}
          >
            A unified system for managing high-value assets and bookings across marketplaces, websites, and offline channels — eliminating double-sells and manual sync work.
          </motion.p>
        </motion.div>
        ...
        >
          ...
          <ul className="space-y-3 max-w-2xl mx-auto">
            {[
              'Central inventory master that syncs stocks across platforms (web, socials, marketplaces).',
              'Booking/reservation engine with availability rules and conflict checks.',
              'Automated listing updates and availability pushes.',
              'Notifications and workflows for confirmations, cancellations, and rescheduling.',
              'Reporting on utilization, turnover, and lost-booking events.'
            ].map((item, index) => (
              <motion.li
                key={index}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl mx-auto">
            {[
              { icon: <FaBox className="w-4 h-4" />, title: 'Reduced Stock Risk', desc: 'Avoid oversells and stock discrepancies across channels.', stat: 'Zero Oversells' },
              { icon: <FaSyncAlt className="w-4 h-4" />, title: 'Higher Utilization', desc: 'Smarter booking logic means fewer idle assets and better occupancy.', stat: '+30% Use' },
              { icon: <FaClock className="w-4 h-4" />, title: 'Faster Time-to-List', desc: 'New inventory and availability are broadcast automatically.', stat: 'Instant Sync' },
              { icon: <FaBell className="w-4 h-4" />, title: 'Operational Clarity', desc: 'One source of truth for inventory and bookings across teams.', stat: 'Unified View' }
            ].map((benefit, index) => (
              <motion.div
                key={index}
        ...
        >
          <motion.p 
            ...
            variants={sectionVariants}
          >
            Unify your inventory and bookings. Let's eliminate the chaos.
          </motion.p>
          <motion.a
...
export default MultiChannelInventoryBooking